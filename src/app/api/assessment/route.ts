 ...
 * POST /api/assessment
 *
 * Body: AssessmentInput. Builds a WellnessProfile from the
 * guest's Island Journey answers.
 *
 * Primary path: Gemini reads the answers + measures metadata and
 * returns strict JSON. Fallback path (no key / any LLM error):
 * deterministic rule-based scoring so the demo always works.
 * Everything the model returns is validated & clamped server-side.
 * ============================================================ */
...
function buildSystemPrompt(): string {
  return [
    ...
    "A guest has played a concise 10-scene, gamified 'Island Journey' — imagining one day on the island. Each playful choice is a behavioral signal about their real wellbeing.",
    "Read their answers together with the hidden 'measures' metadata for each scene. Infer their wellness profile from patterns across the full answer set, not from a single answer, with warmth, nuance and the calm voice of a luxury wellness host (Six Senses / Kamalaya tone).",
    ...
    "Rules:",
    "- value is 0-100. band MUST be consistent with value: 0-33 low, 34-66 moderate, 67-100 high.",
...
export async function POST(request: Request) {
  ...
  // Compose scores (LLM where valid, fallback otherwise).
  const stress = buildScore(result?.stress, fb.stress, summaryStress(fb.stress));
  const migraine = buildScore(
    result?.migraine,
    fb.migraine,
    summaryMigraine(fb.migraine),
  );
  const mental = buildScore(result?.mental, fb.mental, summaryMental(fb.mental));
